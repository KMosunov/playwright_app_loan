...
test.describe("Loan App Tests", async () => {
  test('TL-20-1 base test', async ({ page }) => {
    ...
    await amountSlider.hover({ force: true, position: { x: amountSliderOffsetWidth/2, y: 0} })
    await page.mouse.up()
    ...
    // await smallLoanPage.open()
    // const periodFromSelect = await smallLoanPage.choosePeriodFromSelect('32')
    // console.log(periodFromSelect)
